...
# Creating a logistic regression model using the same variables used in your linear regression homework.  
...
lgs_regression <- glm(HighWelf ~ CCSITE + FWSITE + HOSITE + ZAGELT25 + ZHISP +
    ...
    YREMP + pyrearn + YRREC + YRKREC + YRRFS + RAYEAR, family=binomial, data=train)
...
tree_train = rpart(HighWelf ~ CCSITE + FWSITE + HOSITE + ZAGELT25 + ZHISP +
    ZBLACK + ZACH05 + ZPUBLICH + MNTHEMP + LTRECIP + YR3KEMP + YR3EARNAV +
    YREMP + pyrearn + YRREC + YRKREC + YRRFS + RAYEAR, data=train_data, method="class")
summary(tree_train)
...
It just so happens, with our dataset and our dependent variable, that the smallest xerror happens to involve 0.0237 splits, which is closer to "no pruning."   
...
bag_model <- caret::train(x, y, 
                          ...
                          tuneGrid=expand.grid(mtry=18) # The total number of predictors we put into our model
                          )
bag_model
...
## Now, let's rerun the codes in lines 309 and 310 with this new threshold in mind:  
...
```
Did bagging improve the accuracy? If not, are you still okay with the results? Why or why not? If it did improve accuracy, are you happy with the results? Why or why not? Do you prefer other performance metrics (i.e. True positive rate, True Negative rate etc)?
...
# Let's set it to sqrt(18) predictors and examine our results. 

set.seed(825)
rf_model <- caret::train(x, y, 
                          method = "rf",
                          trControl = fitControl,
                          tuneGrid=expand.grid(mtry=sqrt(18)) # The total number of predictors we put into our model
                          )
rf_model
...
## Now, let's rerun the codes in lines 369 and 370 with this new threshold in mind:  
...
Choose whichever model you prefer best. The term best here is used loosely, so feel free to determine how to define it and explain your reasons below.  
...
bag_ext = randomForest(HighWelf ~ CCSITE + FWSITE + HOSITE + ZAGELT25 + ZHISP +
    ZBLACK + ZACH05 + ZPUBLICH + MNTHEMP + LTRECIP + YR3KEMP + YR3EARNAV +
    YREMP + pyrearn + YRREC + YRKREC + YRRFS + RAYEAR, data=train_data, mtry=18, ntree=2500) # Adjust this last parameter.
...
(2) Change the number of predictors allowed in the random forest growth. Report on whether it changed the prediction accuracy. First use Mtry=4, then 6. e. Why do you think the accuracy is actually better when you try fewer variables? (think about the concept of decorrelation)
